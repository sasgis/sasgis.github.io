...
!!! info
    Исторически, импорт GPX файлов в базу меток появился значительно позже поиска по трекам. В современной же версии программы, с базой меток в SQLite3 формате, вы можете импортировать все свои GPX треки в базу меток и выполнять поиск по Меткам. При большом количестве меток это будет **гораздо эффективнее**, чем устаревший поиск по GPX трекам. 
