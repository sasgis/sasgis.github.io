...
Наши **[ночные сборки](https://github.com/sasgis/sas.planet.src/releases/tag/nightly)** включают в себя две версии программы:
  ...
  * `SASPlanet.Debug.exe` — **дебажная (отладочная)** версия. Она нужна для поиска и исправления ошибок. Когда в ней происходит сбой, она формирует детальный отчёт, который очень помогает разработчикам.
...
3. **Воспроизведите ошибку.** Выполните те же действия, которые привели к сбою в первый раз. Когда возникнет ошибка, дебажная версия покажет специальное окно с отчётом об ошибке. Закройте его и завершите работу программы.
4. **Найдите файл отчёта на диске.** Дебажная версия автоматически сохраняет отчёт об ошибке в файл **`bugreport.txt`** в той же папке, где находится `SASPlanet.Debug.exe`
...
* `show bug report` — **Показать отчёт об ошибке**. Эта кнопка открывает окно с подробной технической информацией о сбое.
...
Пример окна с подробным отчётом, который появляется после нажатия кнопки **show bug report**:
